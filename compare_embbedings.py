...
HYPERPARAMETERS = {
    # --- CORREÇÃO 1: Removido 'auto' de max_features ---
    "RFR": {'bootstrap': [True, False], 'max_depth': [5, 10, 20, 30], 'max_features': ['sqrt', 'log2'], 'min_samples_leaf': [1, 2, 4], 'min_samples_split': [2, 5, 10], 'n_estimators': [50, 100, 200], 'criterion': ['gini', 'entropy']},
...
def load_data():
    ...
    # Script configurado para rodar com 100% dos dados.
    return df_train["content"], df_train["label"], df_test["content"], df_test["label"]

# ======================================================================================================================================================
# Calcular embedding 
# ======================================================================================================================================================
def calculate_embedding(texts, embedding_name, embedding_path, pooling='CLS'):
    tokenizer = AutoTokenizer.from_pretrained(embedding_path)
    
    # Configurado para usar a CPU local
    device = torch.device("cpu")
    
    # Correção para o mBART
    if embedding_name == "MBART":
        tokenizer.src_lang = "pt_XX"
        print(f"  > [PID: {os.getpid()}] Configurando src_lang para MBART. Usando device: {device}")
    else:
        print(f"  > [PID: {os.getpid()}] Processando {embedding_name}. Usando device: {device}")
    ...
    # Configurado para usar todos os cores da CPU (n_jobs=-1)
    # Mude de -1 para 1 para testar
    results = Parallel(n_jobs=4)(delayed(get_embedding)(text) for text in texts)
...
def main():
    ...
    if EMBEDDINGS_FILE.exists():
        ...
        print(f"Embeddings já processados: {list(embeddings.keys())}")
    results = {}
    ...
    for embedding_name, embedding_path in EMBEDDINGS.items():
        ...
        if embedding_name not in embeddings.keys():
            print(f"Calculando embeddings para {embedding_name}...")
            X_train_embeddings, train_embeddings_time = calculate_embedding(X_train, embedding_name, embedding_path)
            X_test_embeddings, test_embeddings_time = calculate_embedding(X_test, embedding_name, embedding_path)
            ...
            print(f"Salvando embeddings de {embedding_name} em {EMBEDDINGS_FILE}...")
            with open(EMBEDDINGS_FILE, 'wb') as file:
                pickle.dump(embeddings, file)
            print("Salvo.")
        else:
            print(f"Embeddings para {embedding_name} já existem. Pulando cálculo.")
        ...
        # --- CORREÇÃO 2: Gráfico comentado para evitar o "crash" do matplotlib/tkinter ---
        ...
        if not results.get(embedding_name):
            ...
            def train_classifier(model_name, model):
                print(f"Iniciando treino: {embedding_name} com {model_name}")
            ...
            print(f"Salvando resultados de {embedding_name} em {RESULTS_FILE}...")
            with open(RESULTS_FILE, "wb") as f:
                pickle.dump(results, f)
            print("Salvo.")
        else:
            print(f"Resultados para {embedding_name} já existem. Pulando treinamento.")
